...
void draw() {
  ...
  for (int i = 410; i < 800; i += mod) {
    for (int j = 10; j < 400; j += mod) {
      ...
      // Función para obtener el color de la elipse
...
// Función para el color de la elipse
...
void keyPressed() {
  if (key == 'c' || key == 'C') {
    // Cambia el color de la elipse a un color aleatorio
    ColorB = color(random(255), random(255), random(255));
  } else if (key == 'r' || key == 'R') {
    // Poner el color de la elipse al color original
    ColorB = color(200, 0, 0);
  }
}
